...
def makeFeatsCsv(wavePath, outPath, model, maxDur, normalised, cuda=False):
    winlen=0.025; winstep=0.01
    audio_file = AudioSegment.from_wav(wavePath)
    feats = getFeatsFromAudio(wavePath, model, maxDur, normalised, cuda=cuda)
...
def getFeatsFromAudio(wavePath, model, maxDur, normalised, cuda=False): # ADD "DICE" LATER
    # maxDur = 30
    ...
    for audio_file in audio_files:
        ...
        if cuda: sigTensor = sigTensor.cuda()
        if normalised:
...
if __name__== "__main__":
    ...
    parser.add_argument('--cuda', '-cu', help="whether to use cuda or not", default=False)
    ...
    if args.cuda == "False": args.cuda = False
    if args.cuda == "True": args.cuda = True
    if Flag:
        print(main.__doc__)
        parser.print_help()
    else:
        main(args.featsFolder, args.json, args.modelPath, float(args.maxDur), args.normalised, args.cuda)
